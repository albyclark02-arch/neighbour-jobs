...
export default function Home() {
  ...
  const regions = [
    ...
    'Penrith','Liverpool','Blacktown','Campbelltown','Wollongong','Newcastle','Central Coast',
  ...
  const feedJobs = [
    { emoji: '🌿', title: 'Lawn mowing', loc: 'Bondi · 1.2km', time: '~2 hrs', pay: '$35' },
    { emoji: '🐕', title: 'Dog walking', loc: 'Surry Hills · 0.8km', time: '45 min', pay: '$20' },
    { emoji: '🌱', title: 'Plant watering', loc: 'Glebe · 1.7km', time: '20 min', pay: '$10' },
    { emoji: '💻', title: 'iPhone setup help', loc: 'Newtown · 3km', time: '~1 hr', pay: '$25' },
    { emoji: '📚', title: 'Maths tutoring', loc: 'Leichhardt · 2km', time: '1 hr', pay: '$30' },
  ].filter(j => selectedJobs.some(s => {
    if (s === 'mowing' && j.title.includes('mowing')) return true
    if (s === 'dogs' && j.title.includes('Dog')) return true
    if (s === 'plants' && j.title.includes('Plant')) return true
    if (s === 'tech' && j.title.includes('iPhone')) return true
    if (s === 'tutoring' && j.title.includes('tutor')) return true
    return false
  }))
  ...
  if (page === 'home') return (
    <main className="min-h-screen bg-white flex flex-col items-center justify-center p-8 text-center">
      ...
      <div className="flex gap-2 mb-8 flex-wrap justify-center">
        <span className="bg-green-50 text-green-700 text-sm px-3 py-1 rounded-full">🛡️ Parent-safe</span>
      ...
      <p className="text-gray-400 text-sm mt-6">For teens aged 11–17 · Australia</p>
    </main>
  )
...
cat > ~/neighbour-jobs/app/page.tsx << 'EOF'
'use client'
import { useState } from 'react'
  ...
  if (page === 'region') return (
    <main className="min-h-screen bg-white flex flex-col p-6 max-w-sm mx-auto">
      ...
      <div className="relative mb-3">
        ...
        <input value={regionSearch} onChange={e => setRegionSearch(e.target.value)} placeholder="Search all Australian regions..." className="w-full border border-gray-200 rounded-xl pl-9 pr-4 py-3 text-sm outline-none focus:border-green-400"/>
      </div>
      <div className="flex-1 overflow-y-auto max-h-72 mb-4">
        {filtered.map(r => (
          <button key={r} onClick={() => setRegion(r)} className={`w-full text-left px-4 py-3 rounded-xl mb-1 flex items-center gap-2 transition-all ${region===r ? 'bg-green-50 border border-green-200' : 'hover:bg-gray-50'}`}>
  ...
  if (page === 'feed') return (
    <main className="min-h-screen bg-white flex flex-col p-6 max-w-sm mx-auto">
      <div className="flex justify-between items-start mb-3">
        ...
        <button className="w-9 h-9 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center text-lg">🔔</button>
      </div>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
        <span className="bg-green-600 text-white text-xs px-3 py-1 rounded-full whitespace-nowrap">All</span>
        {selectedJobs.filter(j=>j!=='other').map(j => (
          <span key={j} className="bg-gray-100 text-gray-600 text-xs px-3 py-1 rounded-full whitespace-nowrap capitalize">{j}</span>
        ))}
      </div>
      <div className="flex-1">
        {feedJobs.length > 0 ? feedJobs.map((j, i) => (
          ...
          <div className="text-center py-12 text-gray-400">
            ...
            <p className="text-sm">No jobs found for your selections.<br/>Try selecting more job types!</p>
          </div>
        )}
      </div>
      <button onClick={() => setPage('home')} className="mt-4 bg-green-600 text-white py-3 rounded-xl font-medium">+ Post a job</button>
    </main>
  )
